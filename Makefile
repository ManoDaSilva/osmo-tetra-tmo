CFLAGS=-g -O2 -Wall -Wl,--heap=65536 `pkg-config --cflags libosmocore 2> /dev/null` -I.
LDLIBS=`pkg-config --libs libosmocore 2> /dev/null` -losmocore

all: conv_enc_test crc_test tetra-rx float_to_bits tunctl createburst
...
clean:
	@rm -f tunctl float_to_bits crc_test tetra-rx conv_enc_test testenc *.o phy/*.o lower_mac/*.o *.a
